...
export function useLevel1BackendExecution({
  address,
  status,
  wallet,
}: {
  ...
  const [challenge, setChallenge] = useState<Level1Challenge | null>(null);
  ...
  const {
    ...
    backendAuth
      ? (["level1-backend-status", backendAuth.accessToken] as const)
      : null,
    async (): Promise<Awaited<ReturnType<typeof fetchLevel1BackendStatus>>> => {
      return fetchLevel1BackendStatus(backendAuth!.accessToken);
    },
  ...
  return {
    backendAuth,
    backendCompleted: Boolean(backendStatus?.completed),
    backendError,
    backendStatus,
    challenge,
    isBackendLoading,
