...
export function useLevel2BackendExecution({
  address,
  status,
  wallet,
}: {
  ...
  const {
    ...
    backendAuth
      ? (["level2-backend-status", backendAuth.accessToken] as const)
      : null,
    async (): Promise<Awaited<ReturnType<typeof fetchLevel2BackendStatus>>> => {
      return fetchLevel2BackendStatus(backendAuth!.accessToken);
    },
  ...
  const prepare = useCallback(async () => {
    ...
    try {
      ...
      let setup: Awaited<ReturnType<typeof setupLevel2>>;
      try {
        await ensureStarted(auth.accessToken);
        setup = await setupLevel2(auth.accessToken, address);
      } catch (error) {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      let setup:
        ...
        | Awaited<ReturnType<typeof setupLevel2>>;
      try {
        await ensureStarted(auth.accessToken);
        setup =
